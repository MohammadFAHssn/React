...
export default function Table({
  ...
  const filteredData = presentableData.filter((r) =>
    [
      ...headKeys.map(
        (key, i) =>
          r[key].replace(/\s+/g, "").includes(fields[i].replace(/\s+/g, "")) ||
...
function TableBodyRow({
  ...
  return (
    ...
    >
      ...
      {haveSeriesColumn && <div style={{ textAlign: "center" }}>{index()}</div>}

      {headKeys.map((key) => (
        <div
          className={numericFields.includes(key) ? "numeric" : "non-numeric"}
        >
          {r[key]}
        </div>
      ))}
