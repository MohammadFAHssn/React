  ...
  function handleDelete() {
    ...
    setBusinessTripList((businessTripList) =>
      businessTripList.filter(
        (businessTrip) => !selectedBusinessTripsIDs.includes(businessTrip.ID)
      )
    );
  }
      ...
      printBusinessTrips(
        selectedBusinessTrips.map(
          (selectedBusinessTrip) =>
            personnel.filter(
              (p) => p.personnelID === selectedBusinessTrip.personnelID
            )[0]
        ),
        selectedBusinessTrips.map(
  ...
  return (
    <section className="report">
      <Table
        ...
        numericFields={["personnelID", "fromDate", "toDate", "duration"]}
